...
CMLayerGeoJSON.prototype.MouseDown=function(TheView,RefX,RefY) 
{
	...
	if ((this.IsVisible())&&(TheView.GetTool()==CMView.TOOL_INFO)) // check if we where clicked in
	{
	...
	if ((this.GetVisible())&&(this.TheData!=null))
	{
		...
		for (var i=0; i < TheFeatures.length; i++) 
		{
			...
			if (CMUtilities.BoundsOverlap(ViewBounds,this.FeatureBounds[i]))
			{
				...
				var TheFeatureStyle=this.GetFeatureProperty(CMLayer.FEATURE_STYLE,i);
				...
				if (TheGeometry.type=="Point") // draw a point as an icon or a mark
				{
					...
					this.PaintPoint(TheView,i,TheCoordinates[0],TheCoordinates[1],TheIconImage);
...
CMLayerGeoJSON.prototype.PaintSelected=function(TheView) 
{
	if ((this.GetVisible())&&(this.TheData!=null))
	{
		...
		if (this.MouseOverFeatureIndex!=-1) // in a feature
		{
			if (this.GetProperty(CMLayer.MOUSE_OVER_STYLE)!==null)
			{
				TheView.SetStyle(this.GetProperty(CMLayer.MOUSE_OVER_STYLE),false);
		...
		{
			if (this.GetProperty(CMLayer.SELECTED_STYLE)!==null)
			{
				TheView.SetStyle(this.GetProperty(CMLayer.SELECTED_STYLE),false);
